...
function drawRedLights() {
    ...
    directions.forEach(dir => {
        ...
        dirConstructions.forEach(construction => {
            ...
            const hasLeftConstruction = otherConstructions.some(c => c.startKm < construction.startKm);
            const hasRightConstruction = otherConstructions.some(c => c.endKm > construction.endKm);
            
            const isLeftEndOfLine = construction.startStationId === stations[0].id;
            const isRightEndOfLine = construction.endStationId === stations[stations.length - 1].id;
            
            if (!isLeftEndOfLine && hasLeftConstruction) {
                const startStation = stations.find(s => s.id === construction.startStationId);
                const pos = kmToPosition(construction.startKm - rules.offset / 1000, construction.direction);
                drawRedLight(redLightLayer, pos.x, pos.y, 'left', startStation.name);
            }
            
            if (!isRightEndOfLine && hasRightConstruction) {
                const endStation = stations.find(s => s.id === construction.endStationId);
                const pos = kmToPosition(construction.endKm + rules.offset / 1000, construction.direction);
                drawRedLight(redLightLayer, pos.x, pos.y, 'right', endStation.name);
            }
        });
    });
}

function drawRedLight(layer, x, y, side, stationName) {
    ...
    innerCircle.setAttribute('cy', y - 25);
    innerCircle.setAttribute('r', '4');
    innerCircle.setAttribute('fill', '#ffffff');
    layer.appendChild(innerCircle);
    ...
    label.setAttribute('fill', '#dc3545');
    label.setAttribute('font-weight', 'bold');
    label.textContent = '红闪灯';
    layer.appendChild(label);
    
    if (stationName) {
        const stationLabel = document.createElementNS('http://www.w3.org/2000/svg', 'text');
        stationLabel.setAttribute('x', x);
        stationLabel.setAttribute('y', y - 55);
        stationLabel.setAttribute('text-anchor', 'middle');
        stationLabel.setAttribute('font-size', '9');
        stationLabel.setAttribute('fill', '#666');
        stationLabel.textContent = `${stationName}设置`;
        layer.appendChild(stationLabel);
    }
}
...
function checkConflicts() {
    ...
    for (let i = 0; i < constructions.length; i++) {
        for (let j = i + 1; j < constructions.length; j++) {
            ...
            if (hasConstructionConflict(c1, c2)) {
                const conflictKey = [c1.code, c2.code].sort().join('-');
                
                if (!detectedConflicts.has(conflictKey)) {
                    detectedConflicts.add(conflictKey);
            ...
            if (hasProtectionDistanceConflict(c1, c2)) {
                const conflictKey = [c1.code, c2.code].sort().join('-');
                
                if (!detectedConflicts.has(conflictKey + '-distance')) {
                    detectedConflicts.add(conflictKey + '-distance');
                    conflictResults.push({
                        c1: c1.code,
                        c2: c2.code,
                        type: 'distance',
    ...
    checkProtectionZoneConflicts(detectedConflicts);
    
    updateConflictResult();
}

function checkProtectionZoneConflicts(detectedConflicts) {
    for (let i = 0; i < protectionZones.length; i++) {
        for (let j = i + 1; j < protectionZones.length; j++) {
            const p1 = protectionZones[i];
            const p2 = protectionZones[j];
            
            if (p1.startKm < p2.endKm && p2.startKm < p1.endKm) {
                const c1 = constructions.find(c => c.id === p1.constructionId1);
                const c2 = constructions.find(c => c.id === p2.constructionId1);
                
                if (!c1 || !c2 || c1.formIndex === c2.formIndex) {
                    continue;
                }
                
                const canShare = canShareProtectionZone(c1, c2);
                
                if (!canShare) {
                    const conflictKey = [c1.code, c2.code].sort().join('-');
                    
                    if (!detectedConflicts.has(conflictKey + '-protection')) {
                        detectedConflicts.add(conflictKey + '-protection');
                        conflictResults.push({
                            c1: c1.code,
                            c2: c2.code,
                            type: 'protection',
    ...
    for (let i = 0; i < protectionZones.length; i++) {
        const p = protectionZones[i];
        
        for (let j = 0; j < constructions.length; j++) {
            const c = constructions[j];
            
            if (c.id === p.constructionId1 || c.id === p.constructionId2) {
                continue;
            }
            
            if (p.startKm < c.endKm && c.startKm < p.endKm) {
                const ownerC1 = constructions.find(con => con.id === p.constructionId1);
                const ownerC2 = constructions.find(con => con.id === p.constructionId2);
                
                if ((!ownerC1 || ownerC1.formIndex !== c.formIndex) && 
                    (!ownerC2 || ownerC2.formIndex !== c.formIndex)) {
                    const ownerCode = ownerC1 ? ownerC1.code : (ownerC2 ? ownerC2.code : '未知');
                    const conflictKey = [ownerCode, c.code].sort().join('-');
                    
                    if (!detectedConflicts.has(conflictKey + '-protection-construction')) {
                        detectedConflicts.add(conflictKey + '-protection-construction');
                        conflictResults.push({
                            c1: ownerCode,
                            c2: c.code,
                            type: 'protection-construction',
                            message: '防护区域与施工区域冲突'
                        });
                    }
                }
...
function canShareProtectionZone(c1, c2) {
    ...
    const distance = Math.abs(c1.endKm - c2.startKm);
    
    return distance <= 1.5;
}

function hasConstructionConflict(c1, c2) {
    return !(c1.endKm < c2.startKm || c2.endKm < c1.startKm);
}

function hasProtectionDistanceConflict(c1, c2) {
    ...
    for (const r of rule.rules) {
        ...
        } else if (r.minSize === 'station_interval') {
            if (distance < 1.5) {
                return true;
            }
        }
...
function updateConflictResult() {
    const resultDiv = document.getElementById('conflictResult');
    
    if (conflictResults.length === 0) {
        resultDiv.className = 'conflict-result no-conflict';
        resultDiv.innerHTML = '<strong>✓</strong> 未检测到冲突，所有施工安排安全合理';
    } else {
        resultDiv.className = 'conflict-result has-conflict';
        let html = '<strong>✗</strong> 检测到以下冲突：<br>';
        
        conflictResults.forEach((conflict, index) => {
            html += `${index + 1}. ${conflict.c1} 与 ${conflict.c2} - ${conflict.message}<br>`;
        });
...
function toggleZoom(direction) {
    const delta = direction === 'in' ? 0.9 : 1.1;
    const newWidth = viewBoxWidth * delta;
    const newHeight = viewBoxHeight * delta;
...
window.addConstruction = addConstruction;
window.confirmConstruction = confirmConstruction;
window.removeConstruction = removeConstruction;
window.deleteConstruction = deleteConstruction;
...
function exportToPDF() {
    ...
    html2canvas(rightPanel, {
        ...
        const defaultFileName = `施工预想图_${dateStr}_${timeStr}.pdf`;
